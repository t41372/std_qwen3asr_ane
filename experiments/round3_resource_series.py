...
def main():
    ...
    def bind_audio(label, data):
        """Every run in the series must have read the same audio content."""
        fingerprint = data.get("audio_fingerprint")
        if not fingerprint:
            raise ValueError(f"{label} did not record the audio it used")
        if report["audio_fingerprint"] is None:
            report["audio_fingerprint"] = fingerprint
        elif report["audio_fingerprint"] != fingerprint:
            raise ValueError(f"{label} read different audio content from earlier runs")
