...
def main():
    ...
    if args.role == "frontend":
        module = AudioFrontend(config, batch_size=args.batch).eval()
        chunk_frames = module.chunk_frames
        shapes = {
            "mel_features": (args.batch, 1, config["num_mel_bins"], chunk_frames),
            "conv1_mask": (args.batch, 1, 1, (chunk_frames + 1) // 2),
            "conv2_mask": (args.batch, 1, 1, (chunk_frames + 3) // 4),
        }
