...
def test_experimental_batched_graphs_follow_the_chosen_compute_unit(bundle, fake_coreml):  # noqa: F811
    module = sys.modules["coremltools"]
    module.models.CompiledMLModel = module.models.MLModel
    path = bundle / "manifest.json"
    manifest = json.loads(path.read_text())
    manifest["round3_audio_batch"] = {"frontend": 4}
    manifest["files"]["frontend_batched"] = "frontend_batched.mlmodelc"
    path.write_text(json.dumps(manifest))
    runtime = ExperimentalRuntime(bundle, compute_units="cpu_only")
    try:
        units = {model.role: model.compute_units for model in fake_coreml}
        assert units["frontend_batched"] == units["frontend"] == "cpu_only"
    finally:
        runtime.close()
