...
- An ANE+GPU variant was measured but is not part of the plugin: a Qwen3-ASR-0.6B draft on the GPU through MLX with exact verification on the ANE produced token-identical output on 200 evaluated utterances at about 2× the speed of the plugin's serial path and 38% less whole-machine energy, at the cost of a second model in memory, a busy GPU and a dependency conflict (`mlx-audio` needs transformers 5). It lives under `experiments/` with its own lock file; see the results file.
