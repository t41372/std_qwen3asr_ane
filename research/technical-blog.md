...
這份紀錄按時間順序更新，包括失敗的路線。成功條件是 Standard ASR main 協議可用、主要模型計算有 ANE 執行證據、辨識品質有對照、效率與耗能可重現；Core ML 格式本身不是 ANE 成功證據。

**先讀這裡。** 這份紀錄是工作日誌，寫給正在做的人看，術語很多。要知道專案是什麼和現在的狀態，先讀 [README](../README.md)；要看數字，讀 [量測結果](results-2026-09-13.md)；不認識的詞查 [名詞解釋](glossary.md)。下面是紀錄裡最常出現的幾個縮寫：

- ANE：Neural Engine，Apple 晶片裡專門跑 AI 模型的低功耗單元。
- T1 / T16：解碼器一次處理 1 個或 16 個字的 Core ML 圖。
- partition：解碼器 28 層被切成的一個 Core ML 檔案。p14 = 切成 2 個檔案、各 14 層；原本是 7 個檔案、各 4 層。
- FP16：未壓縮的權重。LUT8 g32：查表壓成 8 位元、每 32 個輸出通道一張表；LUT4 g16 同理 4 位元。
- smoke：Qwen 官方的兩段測試錄音（英文 15 秒、中文 4 秒）。selection set：挑方案用的 200 句。held-out：只做最後驗證的另 200 句。
- 牆鐘：實際耗時。
- 草稿 / speculative decoding：小模型（0.6B）先猜十幾個字、大模型（1.7B）一次驗證，結果和大模型逐字算相同。
...
「主要在 ANE」在草稿模式下的定義：預先登記要求先定義再量，我是看過 smoke 結果後才定義的，這點如實記錄。三個指標都報：（1）ANE 忙碌牆鐘占比，用同一 PID 的 Instruments ANE 與 GPU 區間：只跑草稿路徑的孤立 trace（`benchmark_mlx_draft.py --skip-serial`，兩段 smoke 各 1 warmup + 1 次）以 ANE Prediction 首尾為工作窗 1.703 s，ANE 區間聯集 1.041 s（61.1%）、窗內 GPU 區間聯集 0.337 s（19.8%，窗前 0.015 s 載入階段排除），其餘約 19% 是 CPU 側；（2）名目 FLOPs 占比：目標 1.7B 處理 7018 個提議＋559 個錨點 token、草稿 0.6B 跑 7200 步，ANE 約 75%；（3）權重位元組流量占比：ANE 每次驗證串流一次 1.7 GB，草稿每步約 0.3 GB，ANE 約 31%——按流量算，草稿模式的多數位元組在 GPU。這條路的代價：依賴 MLX 與 transformers 5（`mlx-audio` 0.5.3 要求 `transformers>=5.14`，與 `convert` 群組的 `<5` 衝突，只能在獨立環境 `experiments/mlx_draft/` 跑）、GPU 不再空閒、系統 wired 記憶體約 2.5 倍。它沒有併入 plugin：預設路徑仍是純 ANE，草稿路徑以可重現的實驗與 lock file 保留，是否作為選配功能是產品決定。
...
## 2026-09-13 — 28：比較集審視、同品質對照、草稿路線做成選配功能

審視第 24 到 27 節的比較集，兩個問題成立：（1）預先登記要求「候選用量化權重時加入 MLX 同位元版本作 baseline」，但 MLX 8-bit 只量了 smoke latency，沒有品質、能耗、記憶體；（2）速度與能耗表裡沒有任何 GPU 路線有錯誤率，所以「GPU 比較快」從來不是「同品質下比較快」，而拿來當最快對照的 MLX 4-bit 品質未知。另外能耗表混了三個場次。以下補齊，門檻不變（加 baseline 是門檻本來的要求）。

同品質對照（`experiments/workflows/quality_matrix.sh`，selection 與 held-out 各 200 句，同語料、normalizer、language auto、256 token）：MLX 8-bit 對 ANE LUT8 g32 四格的配對差都在 0.1 pp 內（selection EN −0.048、ZH +0.027；held-out EN +0.084、ZH 0.000），是同位元、同品質的對照組。MLX 4-bit 的中文 selection +0.983 pp [+0.345, +1.694]、held-out +0.535 pp，與本專案淘汰的 LUT4 同一種退化；它的速度是用中文精度換的，之前的表沒有標明，已更正。MLX bf16 與官方 MPS bf16 對官方 CPU FP32 的差都在 ±0.3 pp 內。壓縮方法不同要說一次：MLX 8-bit 是 decoder 線性量化 group 64、head 不壓；LUT8 g32 是查表、decoder 與 head 都壓。

對「所有性能提升都靠量化」的回答：serial ANE 路線是。相對 FP16 ANE，27% 來自 8-bit palettization（ANE 解壓按元素數計費的快速路徑）、約 5% 來自 partition 從 7 個減到 2 個；serial 解碼受每元素權重解壓限制，其他嘗試（cache 寫入、fused 投影、SDPA、grouped、T64 生成）都沒有動到數字。唯一不是量化的收益是 draft-and-verify 的 2 倍，它是演算法上的、輸出不變。

草稿路線做成選配功能（commit `cdd3c7c`）：`MLXDraft` 與 `TargetCursor`／`VerifyHead` 搬進套件（`draft.py`、`runtime.py`），`CoreMLRuntime.transcribe_speculative` 是批次用的第二條路徑，串流不變；plugin 新增 `draft_dir`／`draft_lookahead`／`draft_bits`，`_artifact_requirements` 多報一個 draft bundle 需求，缺 MLX 時拋 `ConfigError` 並指名 extra。`qwen3-asr-ane build-draft` 從目標 bundle 的 manifest 讀壓縮設定，用 1.7B 來源權重建 T16 compact head、同設定 palettize、compile，連同 pinned 的 0.6B checkpoint 寫成 draft bundle，manifest 記錄目標的 revision、壓縮設定、token 寬度與 tokenizer sha256，載入時逐項核對，另以零狀態探測兩個 head 的 chunk 大小與數量。依賴：`[project.optional-dependencies] gpu-draft = ["mlx-audio[stt]==0.5.3"]` 與 `[tool.uv] conflicts = [[{extra="gpu-draft"},{group="convert"}]]`，跑草稿用 `UV_PROJECT_ENVIRONMENT` 指向第二個 venv；`experiments/mlx_draft/` 獨立環境刪除，量測工具（`benchmark_energy.py`、`benchmark_mlx_draft.py`、`evaluate.py --draft-dir`、`measure_system_memory.py --draft-dir`）全部改走套件路徑，之後的數字量的是出貨的程式。build backend 依使用者要求由 hatchling 換成 `uv_build`，`uv lock --offline`、`uv sync --frozen` 與 307 個測試通過。

套件路徑的一致性：透過 Standard ASR 引擎（`draft_dir` 設定）跑 selection 與 held-out 各 200 句，400 句文字與預設 bundle 的 serial 路線逐句相同、0 錯誤（`experiments/workflows/draft_plugin_parity.sh`）；smoke 兩段 EN 0.606 對 1.419 s（2.34×，接受 45/58）、ZH 0.20 對 0.35 s，token 相同。

文件：README 改回英文、移除讀者設定與工作輪次敘述，補 Apache-2.0 LICENSE、刪除兩份 handoff、`experiments/mlx_draft`；README 與 results 的表以同品質對照為準。同場次的能耗矩陣（FP16、LUT8 7 partition、p14、MLX bf16／8-bit／4-bit、套件草稿路徑，`energy_matrix.sh`）第一次啟動時閒置功率 16 W：電池從 80% 掉到 72% 後開始充電，PSTR 包含充電功率，與第 25 節「充電時不可比」相同，停下重來，加了閒置功率門檻（30 秒平均 < 6.5 W）等充飽再跑；結果見下一節。

獨立審查（整合 delta，唯讀）的結論與修正：（1）`benchmark_speculative.py` 改用 `TargetCursor` 時仍傳整個 `PreparedPrompt` 而非 `.states`，每次執行都會在第一次驗證步 `TypeError`——已修，`p14_and_draft_evidence.sh` 殘留的舊 import 也改走 `measure_system_memory.py --draft-dir`；（2）綁定缺口：verify head 的權重從未與目標 `lm_head` 的權重比對，manifest 只綁 revision／scheme／bits／group_size，所以目標只壓 decoder 不壓 head、或同設定不同 palette 演算法、或 `residual_scale≠1` 都會靜默載入——已修：build 時只在 `lm_head` 屬於壓縮 roles 才壓 head、傳入 `residual_scale`，build 完比對 compiled head 與目標 `lm_head` 的 `weight.bin` sha256 不同就拒絕寫 bundle，載入時再比一次（目前兩者相同：`bdab21e7…`，314 MB）；（3）chunk 大小只查數量不查大小——載入時核對 manifest 的 `vocabulary_chunk` 與探測值；（4）「輸出不變」是量出來的不是結構保證（首字走 bundle `lm_head` 的 1 寬圖與 numpy argmax，其餘走 16 寬 compact head 的裝置端 argmax）——文件改寫為「400 句量到逐 token 相同」，`draft_plugin_parity.sh` 是發布門檻；（5）生命週期：MLX 缺席時 Core ML head 已載入卻不關、MLX 載入失敗不關 head、plugin close 的 draft 失敗會跳過 runtime close、draft 失敗時 runtime.close 例外會蓋掉原錯誤——已修（先 `import mlx.core` 再載 head、try/except 關 head、close 用 try/finally、cleanup 例外用 `add_note`）；（6）串流與 `prepare()` 也會載入用不到的 draft——保留，文件明說「設定即載入」；（7）`lookahead ≥ token_batch_size` 原本每次請求才以 `ValueError` 失敗並被包成「inference failed」——改在載入時拋 `ConfigError`，`draft_lookahead` 下限改 1；（8）`VerifyHead` 沒有測試——加兩個 chunk 的假 head 測試索引運算；（9）載入時核對 draft checkpoint 的 model_id 與 revision，`MIN_SAMPLES` 改從 `audio.py` 匯入；（10）`benchmark_energy.py` 記錄直譯器與套件版本（兩個 venv 的 coremltools 9.0／numpy 2.5.3 相同，Python 3.12.11 對 3.12.13）。審查以 `uv build --offline` 驗證 wheel 含 git direct dependency、`gpu-draft` extra 與兩個 entry point，`uv sync --dry-run --extra gpu-draft --group convert` 確實被 conflicts 拒絕；`ConfigError` 用於「MLX 未安裝」符合 Standard ASR 的錯誤所有權定義。修正後 308 個測試通過，draft bundle 重建、CLI 走草稿路徑正常。

Standard ASR 的 artifact lifecycle 原本沒有實作（`supports_explicit_acquisition=False`，README 教使用者手動跑四個指令），這是缺口不是設計。改為 `supports_explicit_acquisition=True`：`_acquire_artifacts` 在 `<model_dir>.work/` 裡跑量測過的配方（download pinned revision → build T16／p14 → palettize LUT8 g32 → compile 到 `model_dir`）然後刪除中間產物，`draft_dir` 有設就接著 `build_draft_bundle`；`can_acquire_now` 只在轉換工具鏈（torch／transformers／huggingface_hub／safetensors，用 `find_spec` 不匯入）可用且（來源已在或允許下載）時為真，否則 blocker 為 `action_required`（`install_external` 指向 `uv sync --group convert`）或 `downloads_disabled`；incomplete／corrupt 目錄不刪、要求使用者移走。`may_acquire_during_inference` 仍為 false。假轉換函式的測試覆蓋配方參數、工作目錄清理、進度階段順序、失敗時的結構化錯誤與不留半成品；真機 `standard-asr pull std-qwen3asr-ane/1.7b --set model_dir=artifacts/qwen3-asr-1.7b-pulled` 12:37:20 起、1 分 47 秒完成（來源已下載），五個 `weight.bin` 的 sha256 與出貨 bundle 全部相同，轉錄正常；測試 310 個通過，compliance run 通過。

## 2026-09-13 — 29：同場次耗電矩陣

七條路線（ANE FP16、LUT8 7 partition、p14、MLX bf16／8-bit／4-bit、套件草稿路徑）一場次量完，等工作量 40 輪 × 2 smoke、正序再反序、獨立程序，`experiments/workflows/energy_matrix.sh`，原始檔 `artifacts/power-matrix/`。場次條件：電池 80%、AC、不充電，開跑前 30 秒閒置 6.12 W。前面等了一個多小時：第一次啟動閒置 16 W（電池 80%→72% 後充電，PSTR 含充電功率）作廢；充飽後閒置仍 8 到 10 W（機器有其他活動，WindowServer 40%），門檻由 6.5 放寬到 8.5 W，最後一次探測 5.9 W 才開始。

gross J／音訊秒（兩個 block）：FP16 3.277／3.310、LUT8 2.365／2.394、p14 2.266／2.328、MLX bf16 1.950／2.019、MLX 8-bit 1.531／1.553、MLX 4-bit 1.154／1.061、草稿 1.413／1.412；above-idle：2.46／2.50、1.77／1.80、1.70／1.77、1.78／1.85、1.41／1.43、1.06／0.97、1.16／1.16。正反兩輪差在 3% 內。

結論改寫：同位元、同品質的對照（MLX 8-bit 對 LUT8 p14）下，ANE serial 路線每秒音訊的整機能量多 49%（2.30 對 1.54 J），above-idle 多 22%。之前「LUT8 與 MLX bf16 相近」是對的，但 bf16 不是同位元對照；對 MLX 8-bit 是明確輸。ANE 的優勢剩下平均功率（25 對 70–79 W）與 process CPU（0.05 對 0.6 核）。套件草稿路徑 1.41 J，比預設少 39%，與 MLX 8-bit 差 8%，輸出逐字相同。README 與 results 的表以這個場次為準，上午的場次保留作門檻紀錄。
