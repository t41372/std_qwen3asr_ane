...
「主要在 ANE」在草稿模式下的定義：預先登記要求先定義再量，我是看過 smoke 結果後才定義的，這點如實記錄。三個指標都報：（1）ANE 忙碌牆鐘占比，用同一 PID 的 Instruments ANE 與 GPU 區間：只跑草稿路徑的孤立 trace（`benchmark_mlx_draft.py --skip-serial`，兩段 smoke 各 1 warmup + 1 次）ANE 區間聯集 1.041 s、GPU 區間聯集 0.352 s，對 1.705 s 總牆鐘為 61.0% 與 20.7%，其餘約 18% 是 CPU 側；（2）名目 FLOPs 占比：目標 1.7B 處理 7018 個提議＋559 個錨點 token、草稿 0.6B 跑 7200 步，ANE 約 75%；（3）權重位元組流量占比：ANE 每次驗證串流一次 1.7 GB，草稿每步約 0.3 GB，ANE 約 31%——按流量算，草稿模式的多數位元組在 GPU。這條路的代價：依賴 MLX 與 transformers 5（`mlx-audio` 0.5.3 要求 `transformers>=5.14`，與 `convert` 群組的 `<5` 衝突，只能在獨立環境 `experiments/mlx_draft/` 跑）、GPU 不再空閒、記憶體加倍。它沒有併入 plugin：預設路徑仍是純 ANE，草稿路徑以可重現的實驗與 lock file 保留，是否作為選配功能是產品決定。
